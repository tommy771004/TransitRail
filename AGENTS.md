...
npm run sync:singapore-stations # Refresh the cached official MRT/LRT directory + Wikipedia i18n mapping
                                # Writes singapore.json (full record) + singapore-menu.json (browser slice)
...
- **`DownloadScraper`** (tier A) — an official machine-readable feed: GTFS, CSV, XML, JSON, or a published download. `OfficialFeedScraper` is the common subclass wrapping one query function (HK/US/DE/FR/BE/NO/CH/MY/SG). Singapore's LTA adapter resolves a 15-minute signed archive link from DataMall's key-protected GTFS endpoint; DataMall currently rejects the stored `LTA_ACCOUNT_KEY` with HTTP 401, so the market keeps its previous snapshot until the credential is renewed. The key-free static ZIP is a frozen copy of one response whose signed link expired minutes after publication — it can only ever 403. HK MTR resolves live "next train" via station **codes** in `hongKongMtr.ts` (`findMtrJourney` matches by station name → code, so names must match exactly).
- **`BrowserScraper`** (tier B) — an official query page driven with Playwright. The UK nightly snapshot uses TfL's Journey Planner page and its stable internal station IDs, and samples fifteen points across each service day. Each sample retries once with a backoff; up to three samples may fail and the day is still published, but the fourth failure stops the sweep immediately and the previous file is preserved. Stopping early is a budget rule as much as an honesty one: a market refusing every request would otherwise spend fifteen timeouts per route-date, and four routes over seven days at that rate overruns the workflow's 90-minute limit — a cancelled job commits nothing, for any country. A sample that never finds the results markup reports what the page actually was — a block page and a slow render are otherwise the same timeout.
...
- **`FrequencyScraper`** (tier D) — Thailand, whose BEM source publishes first train, last train and headway and no departure list. It writes route files with **no departures at all** and cannot be made to write any; what the operator does publish goes to the service-day artifact. Singapore's SMRT first/last-time API is advisory only and feeds the same artifact; its planned timetable is a tier A `DownloadScraper` (above), not this kind.
