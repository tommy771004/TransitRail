...
- **`BrowserScraper`** (tier B) — an official query page driven with Playwright. The UK nightly snapshot uses TfL's Journey Planner page and its stable internal station IDs, and samples fifteen points across each service day. Each sample retries once with a backoff; up to three samples may fail and the day is still published, but beyond that the day is refused as too gappy and the previous file is preserved. A sample that never finds the results markup reports what the page actually was — a block page and a slow render are otherwise the same timeout.
