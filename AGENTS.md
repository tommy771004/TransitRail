...
- **`BrowserScraper`** (tier B) — an official query page driven with Playwright. The UK nightly snapshot uses TfL's Journey Planner page and its stable internal station IDs, samples fifteen points across each service day, and retries one transient page-navigation failure before preserving the previous file.
