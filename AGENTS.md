...
- **`DownloadScraper`** (tier A) — an official machine-readable feed: GTFS, CSV, XML, JSON, or a published download. `OfficialFeedScraper` is the common subclass wrapping one query function (HK/US/DE/FR/BE/NO/CH/MY/SG). Singapore's LTA adapter follows the DataMall static catalogue ZIP into the signed GTFS archive it names — the published archive currently answers 403, so the market keeps its previous snapshot until LTA refreshes the link. HK MTR resolves live "next train" via station **codes** in `hongKongMtr.ts` (`findMtrJourney` matches by station name → code, so names must match exactly).
