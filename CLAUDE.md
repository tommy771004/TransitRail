...
- **`BrowserScraper`** (tier B) — an official query page driven with Playwright. `UnitedKingdomScraper` (TfL Journey Planner) and `UnitedStatesBrowserScraper` (MBTA trip planner) both extend it; each samples points across the operating day rather than trusting one query. A failed sample is not fatal — the sweep keeps what the planner answered and refuses the day only when more than three of the fifteen samples are missing, because a day published with hour-wide holes reads as "no service" rather than as missing data. That fourth failure also stops the sweep: nothing later can rescue the day, and burning fifteen timeouts per route-date against an unreachable provider is what pushes the nightly job past its 90-minute limit, which would discard every other country's fresh data with it.
