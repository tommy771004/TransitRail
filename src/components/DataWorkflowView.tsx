...
export function DataWorkflowView({ params, onBack }: DataWorkflowViewProps) {
  ...
  return (
    <main className="min-h-screen bg-transparent px-4 pb-nav pt-22">
      <section className="mx-auto max-w-md">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h1 className="m3-headline-small text-slate-900 dark:text-white">{t("workflow.title")}</h1>
        ...
        <ol className="mt-5 space-y-2">
          {steps.map((step, index) => {
            ...
            return (
              <li key={step.title} className="m3-card m3-card-large border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                <div className="flex items-center gap-3">
                  <div className="m3-shape-full flex h-10 w-10 shrink-0 items-center justify-center bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                    <Icon className="h-4 w-4" />
                  </div>
                  <div className="min-w-0">
                    <p className="font-mono text-[11px] text-slate-400">0{index + 1}</p>
                    <h2 className="m3-title-medium text-slate-900 dark:text-white">{step.title}</h2>
                  </div>
                </div>
                <p className="m3-card mt-3 break-words bg-slate-50 px-4 py-3 font-mono text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                  {step.body}
                </p>
              </li>
            );
        ...
        {!countryConfig[params.country].connected ? (
          <div className="m3-card m3-card-large mt-4 flex gap-3 border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950">
            <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-600 dark:text-amber-400" />
            <div>
              <h2 className="m3-title-medium text-amber-900 dark:text-amber-300">{t("workflow.adapter_pending")}</h2>
              <p className="mt-1 text-xs leading-relaxed text-amber-800 dark:text-amber-400">{t("workflow.adapter_body")}</p>
            </div>
          </div>
        ) : (
          <div className="m3-card m3-card-large mt-4 flex gap-3 border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-800 dark:bg-emerald-950">
            <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-700 dark:text-emerald-400" />
            <div>
              <h2 className="m3-title-medium text-emerald-900 dark:text-emerald-300">{t("workflow.live_ready")}</h2>
