              ...
              <div className={`flex shrink-0 ${row.animation} whitespace-nowrap motion-reduce:animate-none group-hover:[animation-play-state:paused] group-focus-within:[animation-play-state:paused]`}>
                {[...row.offers, ...row.offers].map((offer, index) => (
                  <a
                    ...
                    className="m3-chip m3-chip-touch m3-state mx-1.5 shrink-0 border border-sky-200 bg-white text-left text-slate-700 dark:border-sky-800 dark:bg-slate-900 dark:text-slate-200"
                  >
                    ...
                    {offer.conversionType || offer.cookieDays !== undefined || offer.commission ? (
                      <span className="m3-chip m3-label-small m3-shape-full min-h-6 bg-amber-50 px-2 text-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
